 ...
 * Key Components:
   ...
   1. **Sampling** – Agentic behavior via recursive interactions (deprecated in 2026-07-28 release candidate)
   1. Elicitation – Server-initiated requests for user input
   1. Roots – Filesystem boundaries for server access control (deprecated in 2026-07-28 release candidate)

* Protocol Architecture:
  * MCP uses a two-layer architecture:
    1. Data Layer: JSON-RPC 2.0 based communication with lifecycle management and primitives
    1. Transport Layer: STDIO (local) and Streamable HTTP with SSE (remote) communication channels
