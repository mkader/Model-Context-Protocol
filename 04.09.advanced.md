...
* Instead of registering each tool, you instead create two handlers per feature type (tools, resources or prompts). So for example tools then only have two functions like so:
...
* this approach creates a great architecture as everything has its place, the *server.ts* is a very small file that only wires up the request handlers and each feature is in their respective folder i.e tools/, resources/ or /prompts.
...
2. Creating a tool - created in its *tool* subdirectory like so:
  ...
  * see here is how we define name, description and input schema using Pydantic and a handler that will be invoked once this tool is being called.
