...
from finance_crew_openai import run_financial_analysis_async
...
async def analyze_stock(query: str) -> str:
    ...
    try:
        out_buffer = StringIO()
        err_buffer = StringIO()
        # Prevent agent logs from polluting MCP stdio output.
        with redirect_stdout(out_buffer), redirect_stderr(err_buffer):
            result = await run_financial_analysis_async(query)
        return result
